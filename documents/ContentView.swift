...
enum BrowseRoute: Hashable {
  case folder(String)
  case subfolder(parentFolderID: String, subfolderID: String)
  case needsReview
}

struct SmartSubfolder: Identifiable, Hashable {
  ...
  static func makeID(
    ...
    let normalizedTitle =
      ...
      .joined(separator: " ")
    return "\(parentFolderID)::\(normalizedTitle)"
  }
...
struct TabBar: View {
  ...
  @ObservedObject private var fileActivity = FileActivityStore.shared
    ...
    TabView(selection: $selectedTab) {
      Tab(
        ...
        ContentView(
          ...
          isLoadingDrive: library.isLoadingDrive,
          statusMessage: library.statusMessage,
          recentFiles: fileActivity.recentFiles(
            in: library.filesByFolder.values.flatMap { $0 }
              + library.needsReviewFiles
          ),
          onRefresh: { await library.synchronize() },
          onFoldersChanged: {
            Task {
      ...
      Tab(
        ...
        BrowseView(
          ...
          subfoldersByFolder:
            $library.subfoldersByFolder,
          filesBySubfolder:
            $library.filesBySubfolder,
          needsReviewFiles:
            $library.needsReviewFiles,
      ...
      Tab(
        ...
        NavigationStack {
          UniversalSearchView(
            ...
            smartFolders: $library.smartFolders,
            filesByFolder: $library.filesByFolder,
            needsReviewFiles: library.needsReviewFiles
          )
        }
...
struct UniversalSearchView: View {
  ...
  var needsReviewFiles: [FolderFile] = []
  ...
  private var folderIDs: [String] {
    smartFolders.filter { folder in
      query.isEmpty || folder.title.localizedCaseInsensitiveContains(query)
    }.map(\.id)
  }

  private var files: [SearchFileResult] {
    let organizedFiles = smartFolders.flatMap { folder in
      filesByFolder[folder.id, default: []].map { file in
        SearchFileResult(folderID: folder.id, file: file)
      }
    }
    let reviewFiles = needsReviewFiles.map { file in
      SearchFileResult(folderID: "needs_review", file: file)
    }
    return organizedFiles + reviewFiles
  }

  private var displayedFiles: [SearchFileResult] {
    guard !query.isEmpty else { return files }
    return files.filter { result in
      result.file.name.localizedCaseInsensitiveContains(query)
        || result.file.details.localizedCaseInsensitiveContains(query)
        || folderTitle(for: result).localizedCaseInsensitiveContains(query)
    }
  }
...
struct ContentView: View {
  ...
  var isLoadingDrive = false
  var statusMessage: String?
  var recentFiles: [FolderFile] = []
  ...
  private var profileDisplayName = AppProfile.defaultDisplayName

  private var isWorking: Bool { isLoadingDrive || isOrganizing }
    ...
    NavigationStack {
      ScrollView {
        VStack(alignment: .leading, spacing: 20) {
          HStack(alignment: .top) {
            ...
            .disabled(isWorking)
            .accessibilityLabel("Refresh Google Drive")
          }
          .padding(.horizontal, 20)
          if isLoadingDrive || statusMessage != nil
            || (showsOrganisingStatus && isOrganizing)
          {
            libraryStatus
              .padding(.horizontal, 20)
          }
          ...
          VStack(spacing: 0) {
            HStack {
              ...
              .accessibilityLabel("Browse all folders")
            }
            .padding(.horizontal, 20)
            LazyVGrid(
              ...
              ForEach($smartFolders) { $folder in
                HomeSmartFolderCard(
                  folder: $folder,
                  onOpen: { onOpenSmartFolder(folder.id) },
                  onEdit: {
      ...
      .overlay {
        if let selectedFolderID,
          let index = smartFolders.firstIndex(where: { $0.id == selectedFolderID })
        {
  ...
  private var libraryStatus: some View {
    VStack(alignment: .leading, spacing: 10) {
      HStack(spacing: 10) {
        ...
        Text(
          isLoadingDrive
            ? "Loading Google Drive…" : isOrganizing ? "Organising your files" : "Library update"
        )
        .font(.subheadline.weight(.semibold))
        Spacer(minLength: 0)
        if isOrganizing && showsOrganisingStatus {
      ...
      if isOrganizing && showsOrganisingStatus {
        ProgressView(value: progress)
          .tint(AppPalette.accent)
        Text("\(organizedCount) of \(totalToOrganize) files")
          .font(.caption.monospacedDigit())
          .foregroundStyle(.secondary)
      }
    ...
    .padding(15)
    .background(.gray.opacity(0.06), in: RoundedRectangle(cornerRadius: 16))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
          HStack(spacing: 12) {
            ForEach(recentFiles) { file in
              RecentFileCard(
                title: file.name,
                type: (file.name as NSString).pathExtension.uppercased(),
                time: FileActivityStore.shared.openedDateDescription(for: file),
                icon: file.icon,
                color: file.color,
                action: { previewedRecentFile = file }
              )
            }
          }
          .padding(.horizontal, 20)
  ...
  private var progress: Double {
    organizingProgress.isFinite ? min(max(organizingProgress, 0), 1) : 0
  }
  ...
  private var greetingDisplayName: String {
    ...
    return trimmed.isEmpty ? AppProfile.defaultDisplayName : trimmed
  }
}
...
struct BrowseView: View {
  ...
  @Binding var subfoldersByFolder: [String: [SmartSubfolder]]

  @Binding var filesBySubfolder: [String: [FolderFile]]
  ...
  private func subfoldersBinding(
    for folderID: String
  ) -> Binding<[SmartSubfolder]> {
    Binding(
      get: {
        subfoldersByFolder[folderID, default: []]
      },
      set: { subfolders in
        subfoldersByFolder[folderID] = subfolders
      }

    )
  }

  private func subfolderFilesBinding(
    for subfolderID: String
  ) -> Binding<[FolderFile]> {
    Binding(
      get: {
        filesBySubfolder[subfolderID, default: []]
      },
      set: { files in
        filesBySubfolder[subfolderID] = files
      }
...
struct SubfolderView: View {
  ...
  @Binding var subfolders: [SmartSubfolder]

  @Binding var filesBySubfolder: [String: [FolderFile]]

  var searchText: String = ""

  private var displayedSubfolders: [SmartSubfolder] {

    let filtered: [SmartSubfolder]
    if searchText.isEmpty
      || folder.title.localizedCaseInsensitiveContains(
        searchText
      )
    {
      ...
      filtered = subfolders.filter { subfolder in
        subfolder.title.localizedCaseInsensitiveContains(
          searchText
        )
          || filesBySubfolder[
            subfolder.id,
        ...
        VStack(alignment: .leading, spacing: 18) {
          HStack {
            VStack(alignment: .leading, spacing: 4) {
              ...
              Text(
                "\(subfolders.count) categories · \(folder.itemCount) files"
              )
              .font(.system(size: 13))
              .foregroundStyle(.secondary)
            }
          ...
          .padding(.horizontal, 20)
          if displayedSubfolders.isEmpty {
            VStack(spacing: 8) {
              ...
              Text(
                "Files will appear here after AI organisation."
              )
              .font(.system(size: 14))
              .foregroundStyle(.secondary)
              .multilineTextAlignment(.center)
            ...
            VStack(spacing: 0) {
              ForEach(
                Array(
                  displayedSubfolders.enumerated()
                ),
                id: \.element.id
              ) { index, subfolder in
                NavigationLink(
                  value:
                    BrowseRoute.subfolder(
                      parentFolderID:
                        folder.id,
                      subfolderID:
                        subfolder.id
                ...
                .buttonStyle(.plain)
                if index != displayedSubfolders.count - 1 {
                  Divider()
                    .opacity(0.18)
                    .padding(.leading, 92)
                }
  ...
  private var subfolderIcon: some View {
    ZStack(alignment: .topLeading) {
      UnevenRoundedRectangle(
        topLeadingRadius: 7,
        bottomLeadingRadius: 8,
    ...
    .frame(width: 61, height: 53)
    .shadow(
      color: folder.bottomColor.opacity(0.12),
      radius: 5,
      y: 3
    )
  }
...
struct NeedsReviewView: View {
  ...
  private func move(
    ...
    guard
      let fileIndex = files.firstIndex(
        where: { $0.id == file.id }
...
struct DetailView: View {
  ...
  private var presentationFolder: FolderCardModel {

    var copy = folder
    copy.title = pageTitle
    copy.itemCount = files.count
    return copy
  }
  ...
  private var displayedFiles: [FolderFile] {
    ...
    if searchText.isEmpty
      || pageTitle.localizedCaseInsensitiveContains(
        searchText
      )
    {
      filteredFiles = files
...
struct FolderFileRow: View {
  ...
  var body: some View {
    Button {
      guard let url = file.webURL else {
        showsDetails = true
        return
      }
      ...
      openURL(url) { accepted in
        if accepted {
          FileActivityStore.shared.recordOpened(file)
        } else {
          showsOpenError = true
        }
      }
    ...
    .frame(height: 73)
    .accessibilityHint(
      file.webURL == nil ? "Shows file details" : "Opens this file in Google Drive"
    )
    .sheet(isPresented: $showsDetails) {
      FilePreviewSheet(file: file)
    }
    .alert("Couldn’t open this file", isPresented: $showsOpenError) {
      Button("OK", role: .cancel) {}
    } message: {
      Text("Try again after checking that Google Drive or a web browser is available.")
    }
  }
}
    ...
    NavigationStack {
      Form {
        Section {
          ...
          LabeledContent("Details", value: file.details)
          LabeledContent("Date", value: file.date)
        }
...
extension DocumentRole {
  ...
  fileprivate var rotation: Double {
    switch self {
    case .leading:
      -3
    case .featured:
